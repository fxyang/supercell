...
-(void) addBatchNodesToLayer:(GameLayer*)_layer;
-(riActor *) addActorWithDictionary:(NSDictionary *) dictionary;
-(cpConstraint *) addJointWithDictionary:(NSDictionary *) dictionary;
-(riActor*) newActorWithDictionary:(NSDictionary *) dictionar;
-(riActor*) newSpriteWithDictionary:(NSDictionary*)spriteProp;
-(NSMutableArray*) newShapesWithDictionary:(NSDictionary*)spPhysic spriteProperties:(NSDictionary*)spProp data:(riActor*)a;
-(void) setSpritePropertiesWithDictionary:(NSDictionary*)spProp forActor:(riActor *)actor;
-(void) setActorPropertiesWithDictionary:(NSDictionary*)actorProp forActor:(riActor*)actor;
-(void) setShapePropertiesWithDictionary:(NSDictionary*)spPhysic forShape:(cpShape*)shapeDef;
-(NSValue*) newJointWithDictionary:(NSDictionary*)dictionary;
@end


static unsigned int signiture = 0;  
...
-(void) initObjects{
    ...
    actorsInStageNoPhysics = [[NSMutableArray alloc] init];
...
-(riActor *) addActorWithDictionary:(NSDictionary *) dictionary {
    ...
    if(actor != nil){
        ...
        //Multi shapes also supported
        if(actor.body != nil){
            NSArray * shapes = [shapesInStage objectForKey:actor.signiture];
            for(NSValue * shape in shapes){
                cpShape * aShape = [shape pointerValue];
                aShape->body = actor.body;
                [_spaceManager addShape:aShape];
            }
...
-(riActor *) newActorWithDictionary:(NSDictionary *) dictionary {
    riActor * actor = nil;
    ...
    actor = [self newSpriteWithDictionary:spriteProp];
    ...
    if(actor != nil){  
        ...
        BodyType bodyType = [[physicProp objectForKey:@"BodyType"] intValue];
        actor.bodyType = bodyType;
        
        actor.signiture = [NSString stringWithFormat:@"%d",signiture];
        signiture++;
        
        if(bodyType == kBodyNoPhysic)
            [actorsInStageNoPhysics addObject:actor];
        else{
            NSMutableArray* shapes = [self newShapesWithDictionary:physicProp spriteProperties:spriteProp data:actor];
            [shapesInStage setObject:shapes forKey:actor.signiture];
...
-(NSMutableArray*) newShapesWithDictionary:(NSDictionary*)spritePhysic
                                   ...
                                   data:(riActor*)actor 
{
    ...
    CGRect velocity = riRectFromString([spritePhysic objectForKey:@"Velocity"]);
    cpFloat vx0 = velocity.origin.x;
    cpFloat vx1 = velocity.origin.y;
    cpFloat vy0 = velocity.size.width;
    cpFloat vy1 = velocity.size.height;
    
    actor.bodyType = bodyType;
    if(bodyType == kBodyKinematic) bodyType = kBodyStatic;
	...
	if(fixtures == nil || [fixtures count] == 0 || [[fixtures objectAtIndex:0] count] == 0){
		...
		if([[spritePhysic objectForKey:@"IsCircle"] boolValue]){
            ...
            if(bodyType == kBodyStatic) body = cpBodyNewStatic();
            else{
                body = cpBodyNew(mass, cpMomentForCircle(mass, innerDiameter, outterDiameter, cpvzero));
                if (vx0 != 0 || vx1 != 0 || vy0 != 0 || vy1 != 0) {
                    cpFloat vx = vx0;
                    cpFloat vy = vy0;
                    if (vx1 != vx0) vx = (arc4random() % (int)(vx1 - vx0)) + vx0;
                    if (vy1 != vy0) vy = (arc4random() % (int)(vy1 - vy0)) + vy0;
                    body->v = ccp(vx, vy);
                }
            }
...
//NEED FIX
-(NSValue*) newJointWithDictionary:(NSDictionary*)joint {
    
	if(nil == joint)
		return 0;
	NSString *aSig = nil;
    NSString *bSig = nil;
    for(riActor * a in actorsInStage){
        if ([a.name isEqualToString:[joint objectForKey:@"ObjectA"]]) {
            aSig = a.signiture;
        }else if ([a.name isEqualToString:[joint objectForKey:@"ObjectB"]])
            bSig = a.signiture;
    }
    
    cpBody* bodyA  = 0;
    NSMutableArray* bodyAArray = [shapesInStage objectForKey:aSig];
    if([bodyAArray count] > 0 && aSig != nil)
    {
    ...
    cpBody* bodyB  = 0;
    NSMutableArray* bodyBArray = [shapesInStage objectForKey:bSig];
    if([bodyAArray count] > 0 && bSig != nil)
    {
...
#pragma mark Init of LevelLoader
...
-(void) addEverythingToSpaceAndGameLayer{
    NSAssert(_gameLayer != nil && _space != nil , @"Set Space and GameLayer for LevelLoader Before running addEverythingToSpaceAndGameLayer."); 
...
-(void) addSpritesToGameLayer{
    NSAssert(_gameLayer != nil, @"Set GameLayer for LevelLoader Before running addSpritesToGameLayer."); 
	[self addBatchNodesToLayer:_gameLayer];
	for(NSDictionary* dictionary in actorDictsArray){
		...
		if(nil != batch){
			...
			if(nil != actor) [batch addChild:actor];
		}
	}
}
...
-(BOOL) removeSpriteOfActor:(riActor*)actor{
	if(actor != nil){
        if([actor bodyType] == kBodyNoPhysic)
            [actorsInStageNoPhysics removeObject:actor];
        else 
		...
		if(nil != batchNode) 
            ...
            NSLog(@"The Actor [%@] was not created using a batch node so it's your responsibility to remove it.",[actor name]);
            return NO;
        }
//        actor = nil;
        return YES;
...
-(BOOL) removeShapeOfActor:(riActor*)actor{
	if(actor != nil){
        [self removeBodyWithActorSigniture:actor.signiture];
        return YES;
	}
	return NO;
}
...
-(BOOL) removeBodyWithActorSigniture:(NSString *)signiture{
	...
	if(0 != shapes){
        ...
        [shapesInStage removeObjectForKey:signiture];
...
-(BOOL) removeJoint:(cpConstraint*) joint{
	...
	for(NSString* key in keys) [jointsInStage removeObjectForKey:key];
...
#pragma mark Remove/Release ALL

-(BOOL) removeAllSprites{	
    
	BOOL removedAll = YES;
    for (riActor * actor in actorsInStage) {
        CCSpriteBatchNode *batchNode = [actor batchNode];
		if(nil != batchNode) 
            [batchNode removeChild:actor cleanup:YES];
        else {
    ...
    for (riActor * actor in actorsInStageNoPhysics) {
        CCSpriteBatchNode *batchNode = [actor batchNode];
		if(nil != batchNode) 
            [batchNode removeChild:actor cleanup:YES];
        else {
    ...
    [actorsInStage removeAllObjects];
    [actorsInStageNoPhysics removeAllObjects];
...
-(BOOL) removeAllBodies{
	...
	BOOL removedAll = YES;
	for(NSString * key in keys){
		removedAll = removedAll == [self removeBodyWithActorSigniture:key];
	}
	return removedAll;		
}
...
-(BOOL) removeAllJoints{
	...
	BOOL removedAll = YES;
	for(NSString* key in keys) removedAll = removedAll == [self removeJointWithName:key];
	return removedAll;	
}
...
//-(cpBody*) bodyWithName:(riActor*)actor
//{
